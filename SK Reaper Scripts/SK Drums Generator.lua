...
--   conventions. The randomiser can be aimed at a few elements only, leaving
--   the rest of the pattern untouched, and it works on the displayed bar.
--   Swing, timing and velocity spread add human feel, and an optional fill
--   can close the eighth bar.
--
...
-------------------------------------------------------------------------------
-- Which elements RANDOM is allowed to rewrite
--
--  One flag per grid row. A row left out is neither cleared nor regenerated.
--  RESET, DUPLICATE and the factory patterns still act on every row.
-------------------------------------------------------------------------------

local rnd_mask = {}
for i = 1, #DRUM_MAP do rnd_mask[i] = true end

local function save_rnd_mask()
  local t = {}
  for i = 1, #DRUM_MAP do t[i] = rnd_mask[i] and "1" or "0" end
  reaper.SetExtState(EXT_SECTION, "rnd_mask", table.concat(t), true)
end

-- Anything that is not an explicit "0" reads as selected.
local function load_rnd_mask()
  local raw = reaper.GetExtState(EXT_SECTION, "rnd_mask")
  for i = 1, #DRUM_MAP do rnd_mask[i] = (raw:sub(i, i) ~= "0") end
end

local function rnd_any()
  for i = 1, #DRUM_MAP do if rnd_mask[i] then return true end end
  return false
end

local function rnd_all()
  for i = 1, #DRUM_MAP do if not rnd_mask[i] then return false end end
  return true
end
...
-- Builds a pattern for the given range of bars. Rows left out of rnd_mask are
-- never touched, neither cleared nor written to.
local function randomize_intelligent(page_start, page_end)
  ...
  for p = page_start, page_end do
    for s = 1, 16 do
      local step = (p * 16) + s
      for i = 1, #DRUM_MAP do
        if en[i] then grid[i][step] = 0 end
      end
    end
    ...
    -- Open hat
    if en[4] and math.random() > 0.5 then
      ...
      for _ = 1, math.random(1, 3) do
        ...
        if not exists then
          ...
          grid[4][(p*16)+pos] = 3
          if en[3] then grid[3][(p*16)+pos] = 0 end
        end
      end
    end

    -- Clap / Rimshot / Toms
    if en[5] and math.random() > 0.7 then
      grid[5][(p*16)+5]  = 4
      grid[5][(p*16)+13] = 4
    end
    if en[6] and math.random() > 0.6 then
      for _ = 1, math.random(1, 4) do
        local pos = math.random(1, 16)
        if pos ~= 5 and pos ~= 13 then grid[6][(p*16)+pos] = 2 end
      end
    end
    if en[7] and math.random() > 0.95 then grid[7][(p*16)+math.random(1,16)] = 4 end
    if en[8] and math.random() > 0.97 then grid[8][(p*16)+math.random(1,16)] = 4 end
  end
end
...
-- "1;bars;swing,timing,spread;fill;steps". The leading version number lets a
-- later format be recognised and ignored rather than misread.
...
-- Auto-fill takes over the second half of this bar, and is offered only once
-- the pattern is at least that long.
local FILL_BAR   = 8
...
local function write_to_reaper(state)
  ...
  -- Removes the selected items and any pattern written here that overlaps the
  -- new one. Moving the edit cursor elsewhere lays a second pattern further
  -- along instead of replacing the first.
...
-- The dice square marking a grid row as included in RANDOM. Amber when it is.
local function rndToggle(id, on)
  local h       = reaper.ImGui_GetTextLineHeight(ctx)
  local x, y    = reaper.ImGui_GetCursorScreenPos(ctx)
  local clicked = reaper.ImGui_InvisibleButton(ctx, id, h, h)
  ...
  reaper.ImGui_DrawList_AddRectFilled(dl, x0, y0, x0 + s, y0 + s, bg, 3)

  local fg     = on and COL_INK or 0x8C8778FF
  local r      = math.max(1.0, s * 0.11)
  local q      = s * 0.27
  local cx, cy = x0 + s * 0.5, y0 + s * 0.5
  reaper.ImGui_DrawList_AddCircleFilled(dl, cx - q, cy - q, r, fg)
  reaper.ImGui_DrawList_AddCircleFilled(dl, cx,     cy,     r, fg)
  reaper.ImGui_DrawList_AddCircleFilled(dl, cx + q, cy + q, r, fg)
...
-- Shortest gap between two velocity steps made with the wheel.
...
-- Picks the pattern back up where it was left in this project. Opening the
-- script and closing it again leaves the project untouched.
...
local function loop()
  ...
  -- Pick up the saved notes on the first frame, before the fields can report
  ...
  if visible then
    ...
    reaper.ImGui_SameLine(ctx)
    -- Nothing selected in the grid means nothing to generate.
    local rnd_ready = rnd_any()
    if not rnd_ready then reaper.ImGui_BeginDisabled(ctx) end
    if reaper.ImGui_Button(ctx, "RANDOM") then
      randomize_intelligent(state.current_page, state.current_page)
    end
    if not rnd_ready then reaper.ImGui_EndDisabled(ctx) end
    ...
    if reaper.ImGui_CollapsingHeader(ctx,
      ...
      -- ARM routes All MIDI Inputs to the target track, so the note buttons
      -- below play through its sampler. Amber when the track is armed.
    ...
    -- The step grid. The left column carries the dice toggle, the colour dot
    -- and the element name, so it widens with the longest name.
    local tog_w      = reaper.ImGui_GetTextLineHeight(ctx) + 6
    local inst_col_w = 84 + tog_w
    for _, inst in ipairs(DRUM_MAP) do
      local tw = reaper.ImGui_CalcTextSize(ctx, inst.name)
      if tw + 30 + tog_w > inst_col_w then inst_col_w = tw + 30 + tog_w end
    end
    ...
    if reaper.ImGui_BeginTable(ctx, "grid", 17,
      ...
      local all_on = rnd_all()
      if rndToggle("##rndall", all_on) then
        for k = 1, #DRUM_MAP do rnd_mask[k] = not all_on end
        save_rnd_mask()
      end
      reaper.ImGui_SameLine(ctx, 0, 6)
      ...
      for i = 1, #DRUM_MAP do
        ...
        reaper.ImGui_PushID(ctx, 8000 + i)
        if rndToggle("##rnd", rnd_mask[i]) then
          -- Alt-click keeps this element and drops the others
          if (reaper.ImGui_GetKeyMods(ctx) & reaper.ImGui_Mod_Alt()) ~= 0 then
            for k = 1, #DRUM_MAP do rnd_mask[k] = (k == i) end
          else
            rnd_mask[i] = not rnd_mask[i]
          end
          save_rnd_mask()
        end
        reaper.ImGui_PopID(ctx)
        ...
        if ui_edit_name == i then
          ...
          reaper.ImGui_SetNextItemWidth(ctx, inst_col_w - tog_w - 6)
        ...
        for s = 1, 16 do
          ...
          -- Right click lowers the velocity, in edit mode only.
          ...
          if state.vel_edit_mode and reaper.ImGui_IsItemHovered(ctx) then
            ...
            if wheel ~= 0 then
              -- Only the wheel direction counts, never the amount it reports:
              -- one notch moves one layer on any machine.
...
-- Releases any held note, hands the track back as it was found and stores the
