...
local CFG = {
  ...
  -- Mode d'envoi vers les cue buses : PRE-FADER / POST-FX
  -- Le musicien entend le signal après les plugins (VSTi inclus),
  -- mais le fader de la console REAPER n'affecte pas le casque.
  SEND_MODE = 3,

  -- Palette de couleurs disponibles pour les cue buses
  PALETTE = {
    { label="Rouge",  r=180, g=60,  b=60  },
    { label="Orange", r=190, g=110, b=40  },
    { label="Jaune",  r=170, g=160, b=40  },
    { label="Vert",   r=60,  g=160, b=80  },
    { label="Cyan",   r=40,  g=160, b=170 },
    { label="Bleu",   r=60,  g=100, b=190 },
    { label="Violet", r=120, g=60,  b=180 },
    { label="Rose",   r=180, g=80,  b=140 },
...
local function track_name(track)
  if not valid_track(track) then return "(invalide)" end
  local _, n = reaper.GetSetMediaTrackInfo_String(track, "P_NAME", "", false)
  return n ~= "" and n or "(sans nom)"
end
...
-- Conversion couleur REAPER → RGBA ImGui
...
-- Retourne la liste des paires stéréo de sorties hardware de la carte son active
...
-- =============================================================================
--  MODÈLE DE PROJET
--  Toutes les pistes du projet sont des sources potentielles,
--  à l'exception du dossier CUES et des cue buses eux-mêmes.
-- =============================================================================
...
function ProjectModel:scan()
  ...
  -- Première lecture : repérer le dossier CUES et les cue buses
  for i = 0, n-1 do
  ...
  -- Deuxième lecture : toutes les autres pistes = sources disponibles
  -- L'ordre suit l'ordre réel des pistes dans REAPER
  for i = 0, n-1 do
    local t    = reaper.GetTrack(0, i)
    local g    = guid_of(t)
    local role = get_ext(t, "SK_CBM_ROLE")
...
-- Liste des cue buses triée selon leur ordre dans REAPER
...
-- Pistes déjà présentes dans un cue (ont un envoi actif vers ce cue)
...
-- =============================================================================
--  MOTEUR DE ROUTING
--  Gère les envois de chaque piste vers chaque cue bus.
--  Mode : PRE-FADER / POST-FX — le casque est indépendant
--  du fader console REAPER, mais reçoit le signal après les plugins.
-- =============================================================================
...
-- Cherche un envoi existant entre deux pistes. Retourne son index ou -1.
...
-- Ajoute une piste dans un cue (évite les doublons)
...
-- Remet les paramètres d'envoi à leurs valeurs correctes pour tous les envois d'un cue
...
-- Assigne une sortie hardware stéréo à un cue bus
...
-- =============================================================================
--  GESTIONNAIRE DE CUES
--  Création, suppression, duplication, renommage des cue buses.
-- =============================================================================
...
-- Corrige la hiérarchie de dossier CUES après chaque opération
...
-- Réparation complète : dossier, paramètres des pistes, envois
...
-- Copie tous les niveaux du mix d'un cue vers un autre
...
-- Ajoute dans le cue les pistes actuellement sélectionnées dans REAPER
...
-- =============================================================================
--  SNAPSHOTS A / B
--  Deux instantanés de mix par cue, sauvegardés dans le projet REAPER.
--  Le rappel restaure volumes, panoramiques et mutes de chaque piste.
-- =============================================================================
...
-- Volume et mute globaux d'un cue (indépendants des faders individuels)
...
-- Applique le volume et le mute global directement sur la piste cue bus
...
-- =============================================================================
--  BARRE D'OUTILS (haut de fenêtre)
-- =============================================================================
...
local function draw_topbar(ctx, cue_mgr, model)
  ...
  if reaper.ImGui_BeginChild(ctx, "topbar", 0, CFG.TOPBAR_H, 0,
    ...
    if colored_button(ctx, "Dupliquer", CFG.COL.STRIP_BG, CFG.COL.STRIP_SEL, CFG.COL.ACCENT) then
      local ng = cue_mgr:duplicate_cue_bus(UI.selected_cue)
      if ng then UI.selected_cue = ng; set_status("Cue dupliqué.") end
    end
    reaper.ImGui_SameLine(ctx, 0, 4)
    if colored_button(ctx, "Supprimer", CFG.COL.REM_BTN, CFG.COL.REM_HOV, CFG.COL.DANGER) then
      cue_mgr:delete_cue_bus(UI.selected_cue)
      UI.selected_cue = nil
      set_status("Cue supprimé.")
    end
    reaper.ImGui_SameLine(ctx, 0, 4)
    ...
    if colored_button(ctx, ma_lbl, ma_col, ma_hov, CFG.COL.MUTE_ON) then
      ...
      set_status(new_state and "Tous les casques coupés." or "Tous les casques ouverts.")
    end
    ...
    if reaper.ImGui_IsItemHovered(ctx) then
      reaper.ImGui_SetTooltip(ctx,
        UI.strip_wide
        and "Mode large — cliquer pour revenir en normal (80px)"
        or  "Mode normal — cliquer pour passer en large")
    end
    if UI.strip_wide then
      ...
      if reaper.ImGui_IsItemHovered(ctx) then
        reaper.ImGui_SetTooltip(ctx, "Largeur des strips en mode large (80–300 px)")
      end
    end
  ...
  if reaper.ImGui_BeginChild(ctx, "sidebar", CFG.SIDEBAR_W, 0, 0) then
    ...
    for _, cue in ipairs(cues) do
      ...
      if is_ren then
        ...
        -- Bouton mute rapide dans la liste
        ...
        if reaper.ImGui_IsItemHovered(ctx) then
          reaper.ImGui_SetTooltip(ctx,
            sb_master.muted and "Casque coupé — cliquer pour rouvrir" or "Couper ce casque")
        end
      end
...
local function draw_cue_header(ctx, cue, cue_mgr, snap, model)
  ...
  if reaper.ImGui_BeginChild(ctx, "cue_hdr", 0, 50, 0,
    ...
    -- Pastille couleur — cliquer pour ouvrir le sélecteur de couleur
    local native = reaper.GetTrackColor(cue.track)
    if native ~= 0 then
      local col_btn = native_to_imgui(native)
      reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Button(),        col_btn)
    ...
    reaper.ImGui_PopStyleColor(ctx, 3)
    if reaper.ImGui_IsItemHovered(ctx) then
      reaper.ImGui_SetTooltip(ctx, "Changer la couleur du casque")
    end
    ...
    if UI.color_popover and UI.color_cue == cue.guid then
      ...
      if col_visible then
        ...
        reaper.ImGui_Spacing(ctx)
        if colored_button(ctx, "Effacer couleur",
    ...
    -- Nom du casque — double-clic pour renommer
    ...
    if is_ren then
      ...
      reaper.ImGui_PopStyleColor(ctx, 1)
      if reaper.ImGui_IsItemHovered(ctx) then
        reaper.ImGui_SetTooltip(ctx, "Double-clic pour renommer")
      end
    ...
    if reaper.ImGui_IsItemClicked(ctx, 1) then
      snap:save(cue.guid, "A"); set_status("Snapshot A sauvegardé.")
    end
    if reaper.ImGui_IsItemHovered(ctx) then
    ...
    if reaper.ImGui_IsItemClicked(ctx, 1) then
      snap:save(cue.guid, "B"); set_status("Snapshot B sauvegardé.")
    end
    reaper.ImGui_SameLine(ctx, 0, 10)
...
-- =============================================================================
--  STRIP DE FADER (un par piste dans le mix)
-- =============================================================================
...
local function draw_fader_strip(ctx, cue_guid, src, routing, fader_h, on_remove)
  ...
  -- Couleur du strip dérivée de la couleur de la piste dans REAPER
  ...
  if reaper.ImGui_BeginChild(ctx, "strip"..sid, strip_w(), strip_h, 1,
    ...
    if on_remove then
      ...
      reaper.ImGui_PopStyleColor(ctx, 3)
      if reaper.ImGui_IsItemHovered(ctx) then
        reaper.ImGui_SetTooltip(ctx, "Retirer "..src.name.." de ce casque")
      end
    ...
    reaper.ImGui_PopStyleColor(ctx, 1)
    -- Afficher le nom complet au survol si tronqué
    ...
    -- Fader vertical + VU-mètre côte à côte
    ...
    if reaper.ImGui_BeginPopupContextItem(ctx, "panrst"..sid) then
      if reaper.ImGui_MenuItem(ctx, "Centrer le panoramique") then np = 0.0; ch_p = true end
      reaper.ImGui_EndPopup(ctx)
    end
    if ch_p then routing:set_pan(cue_guid, src, clamp(np, -1, 1)) end

    -- Valeur du panoramique centrée sous le slider
...
-- =============================================================================
--  ZONE PRINCIPALE : pistes disponibles + mix du casque
-- =============================================================================
...
local function draw_main_zone(ctx, cue, cue_mgr, routing, snap, model)
  ...
  -- Zone gauche : pistes disponibles à ajouter dans le casque
  ...
  -- Zone droite : mix du casque avec faders
  ...
  if reaper.ImGui_BeginChild(ctx, "zone_mix", avail_w - CFG.AVAIL_W - 4, avail_h - 4, 0,
    ...
    if #in_cue == 0 then
      ...
      reaper.ImGui_TextWrapped(ctx,
        "Aucune piste dans ce casque.\n\n"..
        "Ajoutez des pistes depuis la zone\n"..
        "\"Pistes disponibles\" à gauche.")
      reaper.ImGui_PopStyleColor(ctx, 1)
    else
      reaper.ImGui_Spacing(ctx)
      reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), rgba(CFG.COL.TEXT_DIM))
      ...
      if colored_button(ctx, master.muted and "CUE COUPÉ" or "cue coupé",
        ...
        set_status(master.muted and "Casque coupé." or "Casque ouvert.")
      end
      if reaper.ImGui_IsItemHovered(ctx) then
        reaper.ImGui_SetTooltip(ctx, "Couper / ouvrir tout le casque")
      end
      ...
      -- Calcul de la hauteur des faders selon l'espace réellement disponible
      ...
      if removed then
        ...
        set_status("Retirée : "..removed.name)
      end
    end
...
local function draw_welcome(ctx)
  ...
  if reaper.ImGui_BeginChild(ctx, "welcome", w, h, 0) then
    ...
    local lines = {
      ...
      "",
      "1.  Cliquez [+ Nouveau Cue] pour créer un casque.",
      "    Choisissez sa couleur et sa sortie hardware dès la création.",
      "",
      "2.  Sélectionnez le casque dans la liste de gauche.",
      "",
      "3.  Dans 'Pistes disponibles' :",
      "      [+]  ajouter une piste dans ce casque",
      "      [+ Ajouter sélection REAPER]  ajouter les pistes sélectionnées",
      "",
      "4.  Mixez dans la zone de droite :",
      "      Fader vertical       volume (clic droit : reset / couper)",
      "      Slider horizontal    panoramique (clic droit : centrer)",
      "      MUTE                 couper cette piste dans ce casque",
      "      [x]                  retirer la piste du casque",
      "      Master               volume global du casque",
      "",
      "SNAPSHOTS A / B",
      ...
      "",
      "REPAIR",
      "      Si le script semble désynchronisé avec REAPER, cliquez Repair.",
    }
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), rgba(CFG.COL.TEXT_LABEL))
...
local function draw_new_cue_dialog(ctx, cue_mgr, model, routing)
  ...
  local open, keep = reaper.ImGui_Begin(ctx, "Nouveau casque##newdlg", true,
    reaper.ImGui_WindowFlags_NoResize())
  if not keep then UI.show_new_dlg = false end
  if not open then return end

  reaper.ImGui_Text(ctx, "Nom :")
  reaper.ImGui_SameLine(ctx, 0, 6)
  ...
  reaper.ImGui_PopStyleColor(ctx, 3)
  if reaper.ImGui_IsItemHovered(ctx) then
    reaper.ImGui_SetTooltip(ctx, "Aucune couleur")
  end
  reaper.ImGui_SameLine(ctx, 0, 6)
  ...
  -- Sortie hardware
  reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), rgba(CFG.COL.TEXT_DIM))
  reaper.ImGui_Text(ctx, "Sortie HW :")
  reaper.ImGui_PopStyleColor(ctx, 1)
  reaper.ImGui_SameLine(ctx, 0, 6)
  local hw_opts = get_hw_out_options()
  reaper.ImGui_SetNextItemWidth(ctx, 200)
  if reaper.ImGui_BeginCombo(ctx, "##newhw", hw_opts[UI.new_hw_ch+1] or "Aucune") then
  ...
  if colored_button(ctx, " Créer ", CFG.COL.ACCENT, 0x7AAFFFFF, 0x3A6FC0FF, 120, 28) then
    if UI.new_name_buf ~= "" then
      ...
      UI.selected_cue = g
      set_status("Casque créé : "..UI.new_name_buf)
      local n = 0
  ...
  reaper.ImGui_SameLine(ctx, 0, 8)
  if colored_button(ctx, " Annuler ", CFG.COL.STRIP_BG, CFG.COL.STRIP_SEL,
...
local function draw_copy_from_dialog(ctx, cue_mgr, model)
  ...
  local open, keep = reaper.ImGui_Begin(ctx, "Copier le mix depuis…##cpydlg", true,
    reaper.ImGui_WindowFlags_NoResize())
  if not keep then UI.copy_from_dlg = false end
  if open then
    reaper.ImGui_Text(ctx, "Choisir la source :")
    reaper.ImGui_Spacing(ctx)
    for _, cue in ipairs(model:cue_list()) do
      if cue.guid ~= UI.selected_cue then
        if reaper.ImGui_Selectable(ctx, cue.name.."##cp"..cue.guid, false) then
          ...
          set_status("Mix copié depuis "..cue.name)
          UI.copy_from_dlg = false
        end
      end
    end
    if colored_button(ctx, " Annuler ", CFG.COL.STRIP_BG, CFG.COL.STRIP_SEL, CFG.COL.TEXT_DIM) then
      UI.copy_from_dlg = false
    end
    reaper.ImGui_End(ctx)
  end
...
local function draw_hw_out_dialog(ctx, routing, model)
  ...
  local open, keep = reaper.ImGui_Begin(ctx, "Sortie hardware##hwdlg", true,
    reaper.ImGui_WindowFlags_NoResize())
  if not keep then UI.hw_out_dlg = false end
  if open then
    reaper.ImGui_Text(ctx, "Paire stéréo de sortie :")
    reaper.ImGui_Spacing(ctx)
    local opts = get_hw_out_options()
    opts[1] = "Aucune (désactivé)"
    reaper.ImGui_SetNextItemWidth(ctx, 230)
    if reaper.ImGui_BeginCombo(ctx, "##hwcombo", opts[UI.hw_ch_sel+1] or "Aucune") then
    ...
    if colored_button(ctx, " Appliquer ", CFG.COL.ACCENT2, 0x50D0A0FF, 0x30A080FF, 110, 26) then
      ...
      set_status("Sortie hardware mise à jour.")
      UI.hw_out_dlg = false
    end
...
-- Restaurer la largeur de strip de la session précédente
